...
savefig(f'images5th/{file_tag}_gb_study.png')
show()
print('Best results with depth=%d, learning rate=%1.2f and %d estimators, with accuracy=%1.2f'%(best[0], best[1], best[2], last_best))'''


clf = GradientBoostingClassifier(n_estimators=5, max_depth=5, learning_rate=0.1)
clf.fit(trnX, trnY)
prd_trn = clf.predict(trnX)
prd_tst = clf.predict(tstX)
plot_evaluation_results(labels, trnY, prd_trn, tstY, prd_tst)
savefig(f'images5th/{file_tag}_gb_5_5_01.png')
show()
'''
...
savefig(f'images5th/{file_tag}_gb_ranking.png')
